...
if (!file.exists(occ_grid_path)) {
    message("Fetching occurrence counts from GBIF API")
    grid_size <- 0.1
...
## Fetch GBIF bird occurrence density grid for Belize (Aves, classKey = 212) ------------------------
occ_grid_birds_path <- "data/belize_occ_grid_birds.rds"
if (!file.exists(occ_grid_birds_path)) {
    message("Fetching bird occurrence counts from GBIF API")
    grid_size <- 0.1
    lons <- seq(-89.3, -87.4, by = grid_size)
    lats <- seq(15.8, 18.5, by = grid_size)
    grid_birds <- expand.grid(lon = lons, lat = lats)
    grid_birds$count <- purrr::map2_int(grid_birds$lon, grid_birds$lat, function(lon, lat) {
        wkt <- sprintf(
            "POLYGON((%f %f,%f %f,%f %f,%f %f,%f %f))",
            lon, lat, lon + grid_size, lat, lon + grid_size, lat + grid_size,
            lon, lat + grid_size, lon, lat
        ...
        Sys.sleep(1.5)
        tryCatch(
            rgbif::occ_count(geometry = wkt, hasCoordinate = TRUE, taxonKey = 212L),
            error = function(e) 0L
        )
    }, .progress = TRUE)
    occ_grid_birds <- grid_birds |> filter(count > 0)
    saveRDS(occ_grid_birds, occ_grid_birds_path)
    message("Saved to ", occ_grid_birds_path)
} else {
